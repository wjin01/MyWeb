...
public class BoardAuthenticationFilter implements Filter{
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		//게시글, 수정, 삭제에 대한 필터 - 내가 쓴 글은 나만 수정할 수 있음
		//세션값과 클라이언트에서 넘어오는 글쓴이(usrt_id) 값을 서로 비교
		//수정, 삭제 요청이 들어올 때는 작성자(writer) 정보를 넘겨줌
		...
		//클라이언트에서 넘어오는 id값과 세션 id값을 비교
		
		String writer = request.getParameter("writer");
		
		//작성자가 null이거나 세션값과 같지 않다면
