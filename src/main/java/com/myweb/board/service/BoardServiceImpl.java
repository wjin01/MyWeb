...
//		request.getRequestDispatcher("board_list.jsp").forward(request, response);;
		...
		BoardMapper board= sql.getMapper(BoardMapper.class); //호출 시킬 인터페이스명
	...
	public void update(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result == 1) { //업데이트 성공
			...
			//다시 수정화면으로
			response.sendRedirect("getContent.board?bno=" + bno); //content는 bno를 필요로함
		}
	}
	...
	public void delete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 		- insert, update의 적용 방식이랑 같음
		 * 3. 서비스 영역에서는 bno를 이용해서 삭제를 진행하고 삭제 후에는 목록페이지로 이동
		 */
		...
		if(result == 1) { //삭제 성공
			...
			out.println("<script>");
			out.println("alert('글을 찾을 수 없습니다');");
			out.println("location.href='list.board';");
			out.println("</script>");
		}
